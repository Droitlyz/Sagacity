local library = {
	Version = "0.35",
	WorkspaceName = "Sagacity Dev Premium Scripts",
	flags = {},
	signals = {},
	objects = {},
	elements = {},
	...
	colors = {
		main = Color3.fromRGB(40, 176, 245 ),
		background = Color3.fromRGB(40, 40, 40),
		outerBorder = Color3.fromRGB(15, 15, 15),
		innerBorder = Color3.fromRGB(170, 170, 170),
				...
				if getsynasset then
					...
					elseif (#image > 14) and (string.sub(image, 1, 14) == "synasseturl://") then
						local x, e = pcall(function()
							...
							local fold = isfolder(".Sagacity Dev Premium Scripts")
							if fold then
							else
								makefolder("./Sagacity Dev Premium Scripts")
							end
							fold = isfolder("./Sagacity Dev Premium Scripts/Themes")
							if fold then
							else
								makefolder("./Sagacity Dev Premium Scripts/Themes")
							end
							fold = isfolder("./Sagacity Dev Premium Scripts/Themes/SynapseAssetsCache")
							if fold then
							else
								makefolder("./Sagacity Dev Premium Scripts Themes/SynapseAssetsCache")
							end
							if not fold or not isfile("./Sagacity Dev Premium Scripts/Themes/SynapseAssetsCache/" .. codename .. ".cfg") then
								local res = game:HttpGet(string.sub(image, 15))
								if res ~= nil then
									writefile("./Sagacity Dev Premium Scripts/Themes/SynapseAssetsCache/" .. codename .. ".cfg", res)
								end
							end
							return getsynasset(readfile("./Sagacity Dev Premium Scripts/Themes/SynapseAssetsCache/" .. codename .. ".cfg"))
						end)
						if x and e ~= nil then
							return e
						end
...
function library:CreateWindow(options, ...)
	...
	if windowName and #windowName > 0 and library.WorkspaceName == "Sagacity Dev Premium Scripts" then
		library.WorkspaceName = convertfilename(windowName, "Sagacity Dev Premium Scripts")
	end
	local SagacityLibrary = Instance_new("ScreenGui")
	library.MainScreenGui, MainScreenGui = SagacityLibrary, SagacityLibrary
	...
	SagacityLibrary.Name = "     "
	SagacityLibrary.Parent = library.gui_parent
	SagacityLibrary.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
	SagacityLibrary.DisplayOrder = 10
	SagacityLibrary.ResetOnSpawn = false
	main.Name = "main"
	main.Parent = SagacityLibrary
		...
		function tabFunctions:CreateSection(options, ...)
			...
			if isfolder and makefolder and listfiles and readfile and writefile then
				function sectionFunctions:AddPersistence(options, ...)
					...
					local realDropdownHolderList = Instance_new("UIListLayout")
					local dropdownEnabled = false
					if not isfolder("./Sagacity Dev Premium Scripts") then
						makefolder("./Sagacity Dev Premium Scripts")
					end
					local common_string = "./Sagacity Dev Premium Scripts/" .. tostring(custom_workspace or library.WorkspaceName)
					local function resolvelist(nofold)
						if custom_workspace ~= options.Workspace then
							custom_workspace = options.Workspace
							common_string = "./Sagacity Dev Premium Scripts/" .. tostring(custom_workspace or library.WorkspaceName)
						end
					...
					local function savestuff(s, get)
						...
						if not get then
							...
							rawfile = string.format("%s/%s.cfg", common_string, filenameddst)
						end
					...
					local function loadstuff(s, jsonmode, silent)
						...
						if not jsonmode then
							...
							filename = string.format("%s/%s.cfg", common_string, filenameddst)
						end
